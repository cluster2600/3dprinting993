...
est de remplacer l'image GPU monolithique par quatre images indépendantes,
...
Les noms `simready-core`, `ovrtx-vlm-cu129`, `physicsnemo-cu129` et
`media-cpu` sont les noms de travail des futures images. Ils ne doivent pas être
...
Le run a fourni quatre enseignements factuels à conserver dans les contrats de
build :
...
flowchart LR
    subgraph GHCR["GHCR - images immuables linux/amd64"]
        ...
        PHYS["physicsnemo-cu129<br/>solveurs et post-traitement scientifique"]
    ...
    subgraph Runtime["Exécutions isolées"]
        ...
        PHYSJOB["Job GPU calcul<br/>PhysicsNeMo en batch"]
    ...
    STORE -->|maillage et cas de charge attestés| PHYSJOB
    PHYSJOB -->|résultats numériques et séries temporelles| STORE
...
| `physicsnemo-cu129` | PhysicsNeMo et dépendances scientifiques CUDA 12.9, sans VLM ni renderer | champs, séries temporelles, résidus, bilans, rapports solveur | Validation expérimentale ou résultat de banc moteur |
...
peut ainsi conserver sa pile PyTorch 2.11 CUDA 12.9, tandis que PhysicsNeMo 2.2
peut conserver sa pile qualifiée indépendamment. Une mise à jour de l'un ne doit
...
l'USD ; il ne remplace pas le solveur PhysicsNeMo. Seul le rôle
`physicsnemo-cu129` exécute les calculs numériques décrits par son contrat de
cas de charge.
...
sequenceDiagram
    ...
    C->>V: Louer physicsnemo-cu129 par digest
    V-->>P: Démarrer une machine GPU
    C->>P: Transférer les entrées attestées
    P-->>S: Résultats numériques et rapports solveur
...
7. `physicsnemo-cu129` vérifie la version de PhysicsNeMo, la version de PyTorch,
   `torch.version.cuda == "12.9"`, un tenseur GPU et un calcul minimal qui écrit
   un rapport. Ce calcul atteste le runtime, pas le moteur.
...
versionné pourra associer les quatre noms d'image à leurs digests amd64, au
...
3. Construire `physicsnemo-cu129` et définir un contrat d'entrée/sortie de
   solveur indépendant d'OVRTX.
4. Construire `media-cpu` et rendre reproductibles les photos, planches et
   films à partir de frames déjà récupérées.
5. Ajouter le lock de stack, les gates GHCR par digest et le contrôleur
   séquentiel qui interdit deux locations GPU coûteuses simultanées.
6. Exécuter un run complet, récupérer toutes les attestations, détruire chaque
