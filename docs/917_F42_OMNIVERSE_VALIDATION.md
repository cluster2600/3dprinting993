# Porsche 917 — F42, contrôle Omniverse et SimReady

## Verdict

La culasse F41 possède maintenant un USD minimal lisible et validé sur la
machine GPU louée, mais **pas un actif SimReady automobile complet**. Les
validateurs NVIDIA confirment l'ouverture de la scène, son prim par défaut, ses
unités, son axe et l'absence d'erreurs de schéma détectées. Ils ne démontrent
aucune résistance mécanique, dissipation thermique ou fabricabilité.

Le statut publié est `needs_rerun`, car deux gates restent rouges : le routeur
officiel CAD vers USD n'a pas publié son résultat canonique et le profil
SimReady demandé n'est pas disponible dans l'environnement.

## Géométrie examinée

| Propriété | Valeur |
|---|---:|
| Meshes | 1 |
| Sommets | 34 313 |
| Triangles | 68 678 |
| Axe | Z |
| Unité | 0,001 m |
| Enveloppe | 119,114 × 206,094 × 82,000 mm |
| Étanchéité après soudure exacte | oui |
| Déplacement dû à la soudure | 0 |

Le STEP privé porte le SHA-256
`b3110e5d6d102c7af865b4f5a8067281ed4b9452e331eb68433e4119d36c609a`.
Le USD soudé privé porte le SHA-256
`f2fdc4083390a6a684d431281308374a561c6238b7428bd8a426ac220c1ae644`.
Ces empreintes donnent une provenance reproductible sans publier la géométrie.

## Deux chemins contrôlés

1. Le chemin officiel NVIDIA a préparé le runtime puis exécuté le prévol, la
   conversion et les validateurs. La conversion crée un USD temporaire, mais
   l'adaptateur refuse sa publication atomique : le convertisseur choisit un
   prim racine variable alors que l'adaptateur attend un chemin canonique.
2. Un chemin de contrôle local a converti la même entrée, soudé uniquement les
   doublons de couture à coordonnées strictement égales, défini
   `/HeadF41Baseline`, puis alimenté les validateurs officiels. Le contrôle USD
   minimal et les validateurs asset, géométrie et schéma physique passent.

Le second chemin prouve qu'un USD assaini est validable. Il ne transforme pas
l'échec du premier chemin en réussite du workflow complet.

## Résultats des validateurs

| Gate | Résultat | Portée |
|---|---|---|
| Prévol conversion/validation | PASS | runtime prêt |
| USD minimal | PASS | scène ouvrable, unités et prim valides |
| Asset Validator | PASS | 0 issue de schéma |
| Geometry Validator | PASS | 0 issue de schéma géométrique |
| Physics Validator | PASS | 0 issue, mais aucune propriété physique |
| Profil SimReady | FAIL | profil absent du runtime |
| Rendu OVRTX natif | BLOQUÉ | pilote Vulkan incompatible |

Le PNG publié est un rendu CPU de contrôle visuel. Il ne représente aucun
champ thermique, contrainte, flux ou couche d'impression.

## Conditions de fermeture

Pour passer cette gate, il faut verrouiller le namespace de sortie dans
l'adaptateur CAD, fournir un profil SimReady automobile versionné, ajouter des
matériaux physiques qualifiés sans les inventer, puis relancer OVRTX sur un hôte
dont le pilote Vulkan est compatible. Les validations CFD, thermiques,
structurelles et LPBF restent des gates indépendantes.
