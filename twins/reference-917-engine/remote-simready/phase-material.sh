...
refresh_budget
if ! timeout --foreground "${PHASE_REMAINING_SECONDS}s" "${USD_PYTHON}" - "${ASSET}" "${INTENT_REPORT}" >>"${PHASE_LOG}" 2>&1 <<'PY'
...
phase_pass "Material Agent terminé; sortie concrète enregistrée pour Physics Agent"
