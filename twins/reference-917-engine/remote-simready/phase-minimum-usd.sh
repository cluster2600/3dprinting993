...
PROXY_REFERENCE_REPORT="${PHASE_ROOT}/validate-material-proxy-minimum.json"
PROXY_REFERENCE_MARKDOWN="${PHASE_ROOT}/validate-material-proxy-minimum.md"
...
MATERIAL_PROXY_REPORT="$(report_child_named "${PRODUCER_REPORT}" material-proxy-f10.json)"
...
phase_add_child_report "${PROXY_REFERENCE_REPORT}"
...
run_logged "${USD_PYTHON}" "${REFERENCE}" "${MATERIAL_PROXY}" \
    --report "${PROXY_REFERENCE_REPORT}" --markdown-report "${PROXY_REFERENCE_MARKDOWN}"
require_passed_report "${PROXY_REFERENCE_REPORT}"
phase_pass "USD complet et proxy statique minimal validés avant affectation de propriétés"
